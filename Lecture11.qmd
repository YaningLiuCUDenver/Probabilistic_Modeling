...
the value of a random offer, and let $\bar{F}(x) = P\{X\gt x\} = \int_x^\infty f(u)du$ be its
...
$$ {#eq-eq5-14}
We now argue that the left-hand side of the preceding is a nonincreasing function of $y$. To do so, note that, with $a^+$ defined to equal $a$ if $a \gt 0$ or to equal $0$ otherwise, we have
$$
